...
struct http11_header_parser {
    ...
    void _extract_headers() {
        ...
        m_headline = UrlDecode(std::string(header.substr(0, pos)));
...
template <class HeaderParser = http11_header_parser> struct _http_base_parser {
    ...
    string_map parse_url_params() {
        ...
        auto query = std::string(url.substr(pos + 1));
        std::istringstream queryStream(query);
        std::string keyValuePair;

        while (std::getline(queryStream, keyValuePair, '&')) {
            auto equalsPos = keyValuePair.find('=');
...
struct http_request_parser : _http_base_parser<HeaderParser> {
    ...
    std::string url() { return this->parse_path(); }

    string_map get_headers() { return this->headers(); }

    string_map params() { return this->parse_url_params(); }
};
