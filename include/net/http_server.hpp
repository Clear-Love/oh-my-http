...
struct http_server : std::enable_shared_from_this<http_server> {
    ...
    struct http_request {
        ...
        void
        write_response(int status, std::string_view content,
    ...
    struct http_router {
        ...
        void do_handle(http_request &request) {
            // 寻找匹配的路径
            //debug(), "request path ", request.url;
    ...
    struct http_connection_handler
        ...
        void do_handle() {
            ...
            m_request.headers = std::move(m_req_parser.get_headers());
            m_request.params = std::move(m_req_parser.params());
        ...
        void do_write(bytes_const_view buffer) {
            return m_conn.async_write(buffer, [self = shared_from_this(),
                ...
                if (ret.error()) {
                    //debug(), "write error, close connection";
                    return;
                }
                auto n = ret.value();
    ...
    void do_accept() {
        return m_listening.async_accept(
            m_addr, [self = shared_from_this()](expected<int> ret) {
                auto connfd = ret.expect("accept");
                http_connection_handler::make()->do_start((&self->m_router),
                                                          connfd);
